...
string will return the error rate of the printer as a string representing 
...
function printerError(s) {
  let len = s.length;
  let bad = s.split('').filter( (e,i) => s.charCodeAt(i) > 'j'.charCodeAt(0)).join('').len
  
  console.log(s.split('').filter( (e,i) => s.charCodeAt(i) > 'j'.charCodeAt(0)).join(''))
  //console.log( `${bad}/${len}` )
}
