...
index N where the sum of the integers to the 
left of N is equal to the sum of the integers 
to the right of N. 
...
function findEvenIndex(arr)
{
  ...
  for( let i = 1; i <= arr.length; i++){
    myArrLeft   = arr.slice(0,i-1)
                     .reduce((a,b) => a+b,0);
    myArrRight  = arr.slice(i)
                     .reduce((a,b) => a+b,0);
    
    // console.log(`${myArrLeft}  ${myArrRight}`)
    if (myArrLeft === myArrRight){
      // console.log(i-1)
      return i-1
    }
  }
